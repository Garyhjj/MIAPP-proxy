...
module.exports = {
    ...
    async updateApplication(query, reqOption) {
        ...
        if (+query.ID === 0) {
            const dayInfos = await baseReq.getServiceDayInfo({
            ...
            if (!query.addServiceLater && !testTime(date, query.END_TIME, targetInfo.PRE_MIN_MINUTE)) {
                return {
                    statusCode: 400,
                    error: '已超时，申请服务失败'
                };
            }
            if (!query.addServiceLater && +targetInfo.REMAIN_NUMBER < 1) {
                return {
                    statusCode: 400,
                    error: '申请服务失败, 无剩余号数'
                };
